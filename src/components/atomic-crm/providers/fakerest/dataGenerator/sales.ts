...
import type { PartyKey, RAFile, Sale } from "../../../types";
import type { Db } from "./types";

// Spread the demo team across the three collaborating parties so the ownership
// grouping is visible in demo mode.
const DEMO_PARTIES: PartyKey[] = [
  "online_matters",
  "marketingbende",
  "groeien_met_ads",
];
...
export const generateSales = (_: Db): Sale[] => {
  const randomSales = Array.from(Array(5).keys()).map((id) => {
    ...
    return {
      ...
      administrator: false,
      disabled: false,
      partij: DEMO_PARTIES[id % DEMO_PARTIES.length],
    };
  });
  return [
    {
