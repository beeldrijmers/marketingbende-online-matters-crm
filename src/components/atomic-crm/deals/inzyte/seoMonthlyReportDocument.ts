...
const metricGroupLabel = (group: SeoMonthlyHeadlineMetric["group"]): string =>
  ({
    ...
    website_context: "Websiteontwikkeling",
    ads: "Advertenties",
...
export const buildSeoMonthlyReportDocument = ({
    ...
    .map(
      (metric) => `
        <article class="metric">
          ...
          <div class="metric-kind">${metricGroupLabel(metric.group)}</div>
        </article>`,
    )
    .join("");
  ...
  const measurementSection = hasMeasurement
    ? `<div class="section-title"><h2>Meetresultaten maand-op-maand</h2><span>Huidige maand tegenover vorige maand</span></div><section class="metrics">${metrics}</section>`
...
<section class="hero"><div><p class="eyebrow">Maandelijkse voortgang</p><h1>${escapeHtml(monthLabel(report.reporting_month))}</h1><div class="client">${escapeHtml(companyName)}</div></div><div class="period-card"><strong>${escapeHtml(dateLabel(report.current_start))} – ${escapeHtml(dateLabel(report.current_end))}</strong><span>${periodDetail}</span></div></section>
...
${measurementSection}
<section class="insight"><h2>${interpretationTitle}</h2>${paragraphs(interpretation)}</section>
