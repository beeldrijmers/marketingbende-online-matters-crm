...
create table public.sales (
    ...
    constraint sales_hourly_rate_nonnegative
...
-- Per-user Gmail OAuth connection. Refresh tokens are encrypted with
-- AES-256-GCM in the gmail-connection edge function and never granted to
-- browser roles. history_id is Gmail's incremental synchronization cursor.
...
create unique index uq__gmail_connections__sales_id on public.gmail_connections using btree (sales_id);

-- Short-lived, one-use CSRF state for the Gmail OAuth callback. Server only.
...
alter table public.gmail_connections
    add constraint gmail_connections_sales_id_fkey foreign key (sales_id) references public.sales(id) on update cascade on delete cascade;

alter table public.gmail_oauth_states
    add constraint gmail_oauth_states_sales_id_fkey foreign key (sales_id) references public.sales(id) on update cascade on delete cascade;
...
-- Provider-independent idempotency ledger for Resend and Gmail. Every provider
-- message is claimed before processing; a conflicting insert means "already
-- handled" and prevents duplicate contact/deal notes. Only edge functions
...
create table public.integration_runs (
    ...
    run_kind text not null check (run_kind in ('manual', 'backfill', 'scheduled')),
...
-- One Inzyte workspace link per assignment. OAuth credentials remain in
-- Inzyte; the CRM only stores identifiers for the correct customer account,
-- GA4 property and optional Google data sources. Browser writes are disabled.
...
-- Durable per-assignment execution history. The edge function only stores a
-- bounded result snapshot so a completed analysis remains reviewable without
-- turning the CRM into an unbounded analytics warehouse.
...
-- A status update, as it was shared with the client.
--
-- The body is a SNAPSHOT on purpose: what the client received must keep saying
-- what it said, even after the deal moves to another phase or someone edits the
-- steps. The token is the whole access credential for the public page, so it is
-- long, unique and revocable, and the row deliberately carries no amounts and
-- no internal notes - only the text that was sent.
create table public.deal_status_updates (
    id bigint generated by default as identity primary key,
    deal_id bigint not null,
    company_id bigint,
    ...
    -- Rendered blocks for the public page: [{ heading, lines: [] }].
...
create unique index uq__deal_status_updates__token
    on public.deal_status_updates using btree (token);
create index deal_status_updates_deal_shared_idx
    on public.deal_status_updates using btree (deal_id, shared_at desc);
create index deal_status_updates_company_shared_idx
    on public.deal_status_updates using btree (company_id, shared_at desc);
create index deal_status_updates_shared_by_idx
    on public.deal_status_updates using btree (shared_by);

alter table public.deal_status_updates
    add constraint deal_status_updates_deal_id_fkey foreign key (deal_id) references public.deals(id) on update cascade on delete cascade;

alter table public.deal_status_updates
    add constraint deal_status_updates_company_id_fkey foreign key (company_id) references public.companies(id) on update cascade on delete set null;

alter table public.deal_status_updates
    add constraint deal_status_updates_shared_by_fkey foreign key (shared_by) references public.sales(id) on update cascade on delete set null;
...
alter table public.inzyte_links
    add constraint inzyte_links_deal_id_fkey foreign key (deal_id) references public.deals(id) on update cascade on delete cascade;

alter table public.inzyte_links
    add constraint inzyte_links_company_id_fkey foreign key (company_id) references public.companies(id) on update cascade on delete set null;

alter table public.inzyte_links
    add constraint inzyte_links_created_by_fkey foreign key (created_by) references public.sales(id) on update cascade on delete set null;

alter table public.inzyte_runs
    add constraint inzyte_runs_deal_id_fkey foreign key (deal_id) references public.deals(id) on update cascade on delete cascade;

alter table public.inzyte_runs
    add constraint inzyte_runs_link_id_fkey foreign key (inzyte_link_id) references public.inzyte_links(id) on update cascade on delete set null;

alter table public.inzyte_runs
    add constraint inzyte_runs_requested_by_fkey foreign key (requested_by) references public.sales(id) on update cascade on delete set null;
