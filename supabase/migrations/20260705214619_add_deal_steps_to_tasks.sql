...
alter table "public"."tasks" add column "source" text not null default 'manual'::text;

alter table "public"."tasks" add column "trello_checkitem_id" text;
...
alter table "public"."tasks" add constraint "tasks_contact_or_deal_check" CHECK (((contact_id IS NOT NULL) OR (deal_id IS NOT NULL))) not valid;

alter table "public"."tasks" validate constraint "tasks_contact_or_deal_check";

alter table "public"."tasks" add constraint "tasks_deal_id_fkey" FOREIGN KEY (deal_id) REFERENCES public.deals(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;

alter table "public"."tasks" validate constraint "tasks_deal_id_fkey";

alter table "public"."tasks" add constraint "tasks_source_check" CHECK ((source = ANY (ARRAY['manual'::text, 'trello'::text]))) not valid;
