...
describe("forwardedParser", () => {
  ...
  describe("stripForwardingHeaderBlock", () => {
    ...
    describe("edge cases", () => {
      it("strip everything when there is only one forwarded block and nothing after", () => {
        const text = [
          "---------- Forwarded message ----------",
          "From: Alice <alice@example.com>",
        ].join("\n");
  ...
  describe("stripSubjectForwardingPrefix", () => {
    ...
    it("returns the subject unchanged when no prefix is present", () => {
      expect(stripSubjectForwardingPrefix("Meeting notes")).toBe(
        "Meeting notes",
      );
    });

    it("handles dash separator instead of colon", () => {
      expect(stripSubjectForwardingPrefix("Fwd - My subject")).toBe(
        "My subject",
      );
    });
  ...
  describe("stripMailSignature", () => {
    it("strips the mail signature when separator is present", () => {
      const text = "Hello,\nThis is the email body.\n-- \nSignature here";
      expect(stripMailSignature(text)).toBe("Hello,\nThis is the email body.");
    });

    it("returns original text when no signature separator is found", () => {
      const text = "Hello,\nThis is the email body.\nBest regards.";
      expect(stripMailSignature(text)).toBe(text);
    });

    it("handles multiple occurrences of the signature separator", () => {
      const text =
        "Hello,\nThis is the email body.\n-- \nFirst signature\n-- \nSecond signature";
      expect(stripMailSignature(text)).toBe("Hello,\nThis is the email body.");
    });
  ...
  describe("getForwardedMailContent", () => {
    it("strips both the forwarding header block and the mail signature", () => {
      const text = [
        "---------- Forwarded message ----------",
    ...
    it("handles text with no forwarding header and no signature", () => {
      ...
      expect(getForwardedMailContent(text)).toBe(text);
    });

    it("handles text with only a forwarding header", () => {
      const text = [
        "---------- Forwarded message ----------",
        "From: Alice <alice@wonderland.com>",
        "",
    ...
    it("handles text with only a mail signature", () => {
      const text = "Hello,\nThis is the email body.\n-- \nBest regards.";
      expect(getForwardedMailContent(text)).toBe(
        "Hello,\nThis is the email body.",
      );
